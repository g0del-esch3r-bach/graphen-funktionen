...
string outputstrcritval(int N,float a){
    int M;
    M=1000*a;
    return "critgraph/g"+to_string(N)+"a0."+to_string(M)+".txt";
}
int main(){
    int N,M,k,i,j;
    float cost,alpha,mincost;
    N=8; //<----------------------- FREE VARIABLES;
    alpha=((float)N+1)/((float)N+4); //<---------------- SUBJECT TO CHANGE
    mincost=100000;
    freopen(inputstr(N).c_str(),"r",stdin);
    freopen(outputstrcritval(N,alpha).c_str(),"w",stdout);
    cin>>N>>M;
